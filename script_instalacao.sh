...
apt install -y curl wget git build-essential unzip || show_error "Falha ao instalar dependências"
show_success "Dependências básicas instaladas"

# Instalar NVM e Node.js 16.x
...
# SSH-PLUS já está instalado (ignorado, pois já existe)
...
mkdir -p $BOT_DIR || show_error "Falha ao criar diretório do bot"
cd $BOT_DIR || show_error "Falha ao entrar no diretório do bot"

# Baixar e descompactar o bot
show_status "Baixando e descompactando o bot WhatsApp"
...
# Mover conteúdo do subdiretório takeshi-bot (se existir) para o diretório principal
...
show_success "Bot WhatsApp baixado e configurado"
...
mkdir -p database || show_error "Falha ao criar diretório de database"
show_success "Diretório de database criado"

# Configurar permissões
show_status "Configurando permissões"
chown -R root:root $BOT_DIR || show_error "Falha ao configurar permissões"
chmod +x $BOT_DIR || show_error "Falha ao configurar permissões de execução"
...
ExecStart=/root/.nvm/versions/node/v16.20.2/bin/node src/index.js
Restart=always
RestartSec=10
...
systemctl daemon-reload || show_error "Falha ao recarregar systemd"
systemctl enable takeshi-bot || show_error "Falha ao habilitar serviço"
...
chmod +x configure-bot.sh || show_error "Falha ao configurar permissões do script de configuração"
